...
public class Demo1 {
	...
	public static String solution1(String s) {
		...
		Map<Integer, String> map = Arrays.stream(s.split(" "))
				.collect(Collectors.toMap(word -> Integer.parseInt(word.replaceAll("[^0-9]", "")), word1 -> word1,
						(oldValue, newValue) -> oldValue, TreeMap::new));
		return map.values().stream().map(word -> word.replaceAll("[^a-zA-Z | \s]", ""))
